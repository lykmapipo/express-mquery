...
var parse = require('auto-parse');
var chai = require('chai');
var sinon = require('sinon');
var sinonChai = require('sinon-chai');
var expect = chai.expect;
chai.use(sinonChai);
var prepareQuery = require(path.join(__dirname, '..', '..', 'lib',
  'prepareQuery'));
...
describe('prepareQuery', function() {
  ...
  afterEach(function() {
    if (next && next.restore && _.isFunction(next.restore)) {
      next.restore();
    }
  });
  ...
  it('calls next when sort key is a string', function() {
    ...
    expect(req.mquery).to.be.eql(parse(req.query));
    expect(next).to.have.been.calledOnce;
    expect(next).to.have.been.calledWithExactly;
  });
