...
//TODO support projection query params
//TODO refactor
 ...
 *              distinct conditions
 * @param {Object} options valid query params options to parse for distinct fields
 * @param {Function} done a callback to invoke on success or failure
 ...
 */
exports.distinct = function distinct(options, done) {

  //try parsing distinct
  try {

    //ensure query
    const query = _.merge({}, parse(options));

    //obtain distinct field
    let distinct = query.distinct;
    distinct = _.isEmpty(distinct) ? undefined : distinct.trim();
 ...
 */
exports.sort = function(options, done) {
 ...
 * @public
 */
exports.paginate = function(options, done) {
  ...
  try {
    ...
    //see https://apigee.com/about/tags/restful-api-design
    const skip =
      (query.skip || query.offset || query.start || 0);
...
exports.parse = function(string, done) {
  ...
  try {
    ...
    async.parallel({
      ...
      populate: function parsePopulate(next) {
        exports.populate(json, next);
      },

      distinct: function parseDistinct(next) {
        exports.distinct(json, next);
      },

      sort: function parseSort(next) {
        exports.sort(json, next);
      },

      paginate: function parsePaginate(next) {
