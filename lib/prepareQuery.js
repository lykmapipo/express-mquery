        ...
        for (i = 0, length = populate.length; i < length; i++) {
          ...
          for (var key in queryOptions.select) {
            if (key.indexOf(populate[i] + '.') === 0) {
              ...
              queryOptions.populate[i].select += key.substring(populate[i].length +
                1);
          ...
          if (queryOptions.select) {
            if (Object.keys(queryOptions.select).length > 0 && !queryOptions.select[
                populate[i]]) {
    ...
    for (var key in request.query) {
      ...
      if (key === 'query') {
        try {
          ...
          var error = new Error(key + ' must be a valid JSON string');
      ...
      //parse to number
        ...
        try {
          request.mquery[key] = parseInt(request.query[key]);
        } catch (e) {
          request.mquery[key] = request.query[key];
        }
      }
