...
const filter = (options, done) => {
  ...
  try {
    // ensure query
    const query = lodash.merge({}, autoParse(options));
    ...
    const filters = lodash.omitBy(
      lodash.merge({}, filterFields, { q }),
...
const headers = (options, done) => {
  ...
  try {
    ...
    let payload = lodash.merge({}, query.header, query.headers);

    // normalize headers

    // ... pick allowed headers
    const ALLOWED_HEADERS = ['if-modified-since'];
    payload = lodash.pick(payload, ALLOWED_HEADERS);

    // ... camelize header names
    payload = lodash.mapKeys(payload, (value, key) => lodash.camelCase(key));

    // ... normalize date headers
    const DATE_HEADERS = ['ifModifiedSince'];
    payload = lodash.mapValues(payload, (value, key) =>
      lodash.includes(DATE_HEADERS, key) ? new Date(value) : value
    );
...
const paginate = (options, done) => {
  ...
  try {
    // ensure query
    let query = lodash.merge({ maxLimit: 50 }, autoParse(options));

    // normalize json-api page params
    if (query.page && lodash.isPlainObject(query.page)) {
      query = lodash.merge({}, query, query.page);
    ...
    let limit =
      ...
      query.count ||
      query.perPage ||
      lodash.get(query, 'per_page') ||
      query.cursor ||
      10;
    limit = limit > query.maxLimit ? query.maxLimit : limit;
    limit = limit < 0 ? 10 : limit;
    paginations = lodash.merge({}, paginations, { limit });

    // parse page
    // see https://apigee.com/about/tags/restful-api-design
    let page = query.page || query.number || 1;
    page = page < 0 ? 1 : page;
    paginations = lodash.merge({}, paginations, { page });

    // parse skip
    // see https://apigee.com/about/tags/restful-api-design
    const hasSkip =
      lodash.has(query, 'skip') || lodash.has(query, 'offset') || lodash.has(query, 'start');
    let skip = query.skip || query.offset || query.start || 0;
    skip = skip < 0 ? 0 : skip;
    // correct skip base on page number
    skip = !hasSkip && page > 0 ? (page - 1) * limit : skip;
    paginations = lodash.merge({}, paginations, { skip });
...
const select = (options, done) => {
  ...
  try {
    ...
    if (lodash.isString(projections)) {
      projections = [].concat(projections);
    }

    if (lodash.isPlainObject(projections)) {
      projections = lodash.map(projections, (proj, key) => {
        if (!lodash.isNaN(Number(key))) {
          return proj;
        }
        return {
          [key]: proj,
    ...
    const fieldify = (projection = '') => {
      // prepare projections
      let fields = lodash.compact(projection.split(','));
      fields = lodash.map(fields, lodash.trim);
    ...
    projections = lodash.reduce(
      projections,
      (accumulator, projection) => {
        // handle string projection i.e select=name,email
        if (projection && lodash.isString(projection)) {
          const fields = fieldify(projection);
          // eslint-disable-next-line no-param-reassign
          accumulator = lodash.merge({}, accumulator, fields);
        }

        if (lodash.isPlainObject(projection)) {
          lodash.forEach(projection, (value, key) => {
            if (!lodash.isNaN(Number(value))) {
              // eslint-disable-next-line no-param-reassign
              accumulator = lodash.merge({}, accumulator, {
                [key]: value,
              });
            } else {
...
const populate = (options, done) => {
  ...
  try {
    ...
    if (populations && lodash.isString(populations)) {
      // ensure unique populations
      populations = [].concat(populations.split(','));
      populations = lodash.compact(populations);
      populations = lodash.map(populations, (population) => population.trim());
      populations = lodash.uniq(populations);

      // normalize populations to object based
      // and
      // map to array of populations
      populations = lodash.map(populations, (population) =>
        lodash.merge({}, { path: population })
      );
    }
    ...
    // ensure array
    populations = lodash.compact([].concat(populations));
...
const sort = (options, done) => {
  ...
  try {
    ...
    if (sorts && lodash.isString(sorts)) {
      // ensure unique sorts
      sorts = lodash.compact(sorts.split(','));
      sorts = lodash.uniq(sorts);
      ...
      (error, payload) => {
        if (payload) {
          ...
          if (payload.populate && payload.select) {
            const populations = lodash.map(payload.populate, 'path');
            // eslint-disable-next-line no-param-reassign
            payload.select = lodash.omit(payload.select, populations);
          }

          // compact
          // eslint-disable-next-line no-param-reassign
          payload = lodash.omitBy(payload, lodash.isEmpty);
        }
